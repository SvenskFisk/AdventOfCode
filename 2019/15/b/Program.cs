...
namespace a
{
    ...
    {
        static int[] oldMoves;
        static HashSet<(int, int)> open = new HashSet<(int, int)>();
        static (int, int) oxyLoc = (0, 0);
        static long[] input;
        static void Main(string[] args)
        {
            ...
            var ox = new List<(int, int)> { oxyLoc };

            for (int i = 1; i < int.MaxValue; i++)
            {
                var added = false;
                foreach (var (x, y) in ox.ToArray())
                {
                    var adjacent = new[]
                    {
                    ...
                    {
                        if (open.Contains(a) && !ox.Contains(a))
                        {
                            ox.Add(a);
                            added = true;
                        }
