...
1 MHQGW, 4 ZVHX => 3 BDKZM";
            ...
            var demands = new Queue<Quantity>(new[] { new Quantity("FUEL", 1) });
            var leftovers = reactions.Keys.Concat(new[] { "ORE" }).ToDictionary(x => x, x => 0);
            ...
            foreach (var d in leftovers.Where(x => x.Value != 0))
            {
                Console.WriteLine(d);
            }
        ...
        struct Quantity
        {
            ...
            public Quantity(string chemical, int mass)
            {
                Chemical = chemical;
                Mass = mass;
            }
